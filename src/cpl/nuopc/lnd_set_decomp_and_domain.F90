...
!===============================================================================
  ...
  subroutine lnd_set_decomp_and_domain_from_readmesh(vm, meshfile_lnd, meshfile_mask, mesh_slim, &
       ni, nj, rc)
    ...
    type(ESMF_Mesh)     , intent(out)   :: mesh_slim
    ...
    type(ESMF_DistGrid) :: distgrid_slim
    integer             :: g,n             ! indices
    ...
    integer  , pointer  :: gindex_slim(:)  ! global index space for land and ocean points
    ...
    ! Determine lnd decomposition that will be used by slim from lndmask_glob
    ...
    ! Determine ldomain%mask and ldomain%frac using slim decomposition
    do g = begg, endg
       n = 1 + (g - begg)
    ...
    ! Generate a slim global index that includes both land and ocean points
    nocn = size(gindex_ocn)
    allocate(gindex_slim(nlnd + nocn))
    do n = 1,nlnd+nocn
       if (n <= nlnd) then
          gindex_slim(n) = gindex_lnd(n)
       else
          gindex_slim(n) = gindex_ocn(n-nlnd)
       end if
    end do

    ! Generate a new mesh on the gindex decomposition
    distGrid_slim = ESMF_DistGridCreate(arbSeqIndexList=gindex_slim, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    mesh_slim = ESMF_MeshCreate(mesh_lndinput, elementDistGrid=distgrid_slim, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

    ! Set ldomain%lonc, ldomain%latc and ldomain%area
    call lnd_set_ldomain_gridinfo_from_mesh(mesh_slim, vm, gindex_slim, begg, endg, isgrid2d, ni, nj, ldomain, rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

    ! Deallocate local pointer memory
    deallocate(gindex_lnd)
    deallocate(gindex_ocn)
    deallocate(gindex_slim)
  ...
  subroutine lnd_get_global_dims(ni, nj, gsize, isgrid2d)
    ...
    ! grid. However, SLIM needs this information in the history file generation via the
    ! logical variable isgrid2d.
  ...
  subroutine lnd_set_lndmask_from_maskmesh(mesh_lnd, mesh_mask, vm, gsize, lndmask_glob, lndfrac_glob, rc)
    ...
    flandfrac = './init_generated_files/slim_landfrac'//trim(inst_suffix)//'.nc'
    ...
    if (lexist) then
       ...
       ! determine global landmask_glob - needed to determine the slim decomposition
       ...
       ! Determine ldomain%frac using both input and slim decompositions
       ! lndfrac_glob is filled using the input decomposition and
       ! ldomin%frac is set using the slim decomposition
  ...
  subroutine lnd_set_ldomain_gridinfo_from_mesh(mesh, vm, gindex, begg, endg, isgrid2d, ni, nj, ldomain, rc)
    ...
    ! Create ldomain%area by querying the mesh on the slim decomposition
