module lnd_comp_nuopc
  ...
  !----------------------------------------------------------------------------
  ! This is the NUOPC cap for SLIM
  !----------------------------------------------------------------------------
...
!===============================================================================
  ...
  subroutine InitializeAdvertise(gcomp, importState, exportState, clock, rc)
    ...
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    if (trim(rof_model) /= 'srof' .and. trim(rof_model) == 'drof') then
       call shr_sys_abort( subname//'ERROR:: SLIM can NOT currently be coupled to a prognostic ROF model' )
    end if
    rof_prognostic = .false.      ! Always assume a ROF model is NOT under SLIM
    ...
    if (trim(glc_model) /= 'sglc') then
       call shr_sys_abort( subname//'ERROR:: SLIM can NOT be coupled to a prognostic GLC model' )
    end if
    glc_present = .false.    ! Always assume a land-ice model is NOT present
    ...
    call advertise_fields(gcomp, flds_scalar_name, atm_prognostic, rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
    ...
    integer                 :: nsrest                ! SLIM restart type
    ...
    if ( single_column )then
       call shr_sys_abort( subname//'ERROR:: single_column NOT setup for SLIM yet' )
    end if
    ...
    if ( nthrds > 1 )then
       call shr_sys_abort( subname//'ERROR:: SLIM can NOT currently handle threading' )
    end if
    ...
    ! Note that we assume that SLIM's internal dtime matches the coupling time step.
    ...
    ! ---------------------
    ! Initialize first phase of SLIM
    ! ---------------------
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    integer                :: ymd            ! SLIM current date (YYYYMMDD)
    integer                :: yr             ! SLIM current year
    integer                :: mon            ! SLIM current month
    integer                :: day            ! SLIM current day
    integer                :: tod            ! SLIM current time of day (sec)
    ...
    do while(.not. dosend)
       ...
       !--------------------------------
       ! Advance SLIM time step
       !--------------------------------

       call t_startf ('lc_slim2_adv_timestep')
       call advance_timestep()
       call t_stopf ('lc_slim2_adv_timestep')

    end do

    ! Check that internal clock is in sync with master clock
    ! Note that the driver clock has not been updated yet - so at this point
    ! SLIM is actually 1 coupling intervals ahead of the driver clock
    ...
    if ( (ymd /= ymd_sync) .or. (tod /= tod_sync) ) then
       write(iulog,*)'slim ymd=',ymd     ,' slim tod= ',tod
       write(iulog,*)'sync ymd=',ymd_sync,' sync tod= ',tod_sync
       call ESMF_LogWrite(subname//" SLIM clock not in sync with Master Sync clock",ESMF_LOGMSG_ERROR)
       rc = ESMF_FAILURE
       return
    end if
    ...
    if (dbug > 1) then
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       if (masterproc) then
          call log_clock_advance(clock, 'SLIM', iulog, rc)
          if (ChkErr(rc,__LINE__,u_FILE_u)) return
       end if
    end if
  ...
  subroutine ModelFinalize(gcomp, rc)
    ...
    if (masterproc) then
       write(iulog,F91)
       write(iulog,F00) 'SLIM: end of main integration loop'
       write(iulog,F91)
    end if
